...
class HybridTaskCascadeAlign(CascadeRCNN):
    ...
    def __init__(self,
                 ...
                 mask_info_flow=True,
                 **kwargs):
        super(HybridTaskCascadeAlign, self).__init__(num_stages, backbone, **kwargs)
    ...
    def _bbox_forward_train(self,
        ...
        mask_targets = bbox_head.get_target_mask(sampling_results,
                                                     gt_masks, 
                                                     rcnn_train_cfg)
    ...
    def forward_dummy(self, img):
        ...
        for i in range(self.num_stages):
            cls_score, bbox_pred, mask_pred = self._bbox_forward_test(
        ...
        for i in range(self.num_stages):
            ...
            loss_bbox, rois, bbox_targets, bbox_pred = \
                self._bbox_forward_train(
                    i, x, sampling_results, gt_bboxes, gt_masks, gt_labels,
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                mask_classes = self.bbox_head[-1].num_classes - 1
                segm_result = [[] for _ in range(mask_classes)]
            else:
                _bboxes = (
                    det_bboxes[:, :4] *
                ...
                aug_masks = []
                mask_roi_extractor = self.bbox_roi_extractor[-1]
                mask_feats = mask_roi_extractor(
                    x[:len(mask_roi_extractor.featmap_strides)], mask_rois)
                if self.with_semantic and 'mask' in self.semantic_fusion:
                    mask_semantic_feat = self.semantic_roi_extractor(
                ...
                last_feat = None
                for i in range(self.num_stages):
                    mask_head = self.bbox_head[i]
                    if self.mask_info_flow:
                        #mask_pred, last_feat = mask_head(mask_feats, last_feat)
                        # TODO, last feat
                        cls_score, bbox_pred, mask_pred = mask_head(mask_feats)
                    else:
                        #mask_pred = mask_head(mask_feats)
                        cls_score, bbox_pred, mask_pred = mask_head(mask_feats)
                ...
                merged_masks = merge_aug_masks(aug_masks,
                                               ...
                                               self.test_cfg.rcnn)
                segm_result = self.bbox_head[-1].get_seg_masks(
